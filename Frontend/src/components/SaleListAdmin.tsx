...
export default function SaleListAdmin({ onEdit, autoAdd = false, onAutoAddProcessed }: Props) {
  ...
  const paginatedSale = filtered.slice(startIndex, startIndex + itemsPerPage);

  // Reset strony gdy zmienia się filtr
  useEffect(() => {
    setCurrentPage(1);
  }, [search, stanowiskaFilter, sortKey]);
            ...
            <div className="list-item-actions mt-auto">
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-2 w-full">
                ...
                >
                  ...
                  <span className="hidden sm:inline">Przejdź do strony sali</span>
                  <span className="sm:hidden">Strona sali</span>
                </button>
                <button
      ...
      {totalPages > 1 && (
        <div className="mt-6 flex items-center justify-between">
          <div className="text-sm text-gray-600">
            Wyświetlanie {startIndex + 1}-{Math.min(startIndex + itemsPerPage, filtered.length)} z {filtered.length} sal
          </div>
