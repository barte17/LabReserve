...
const UnauthorizedMessage: React.FC<UnauthorizedMessageProps> = ({ type, className = '' }) => {
  ...
  const getMessage = (): { title: string; description: string; contact?: boolean } => {
    switch (type) {
      case 'business_roles':
        return {
          title: 'Konto wymaga aktywacji',
          description: 'Aby korzystać z funkcji rezerwacji, Twoje konto musi zostać aktywowane przez administratora.',
          contact: true
        };
      case 'stanowiska':
        return {
          ...
          {contact && (
            <div className="text-sm">
              <p className="text-yellow-700 mb-2">
                Aby uzyskać niezbędne uprawnienia:
              </p>
              <ul className="list-disc list-inside text-yellow-600 space-y-1">
                <li>Skontaktuj się z administratorem systemu</li>
                <li>Wyślij email na adres: <strong>admin@uczelnia.pl</strong></li>
                <li>Napisz do działu IT uczelni</li>
              </ul>
            </div>
          )}
        </div>
